...
#include "OutOfBoundsException.h"	// Header file that contains definition for object called OutOfBoundsException
...
namespace TestNameSpace
{
	namespace Containers
	{
		...
		{
			m_size = s_default_size;  // Set default size of the array to m_size
			m_data = new T[m_size]; // Instantiate the array of T objects

			if (m_data == nullptr)  // Check what new returns to see if its NULL
				std::cout << "Error: memory could not be allocated";
			else
				std::cout << "Array default constructor." << endl;
		}

		// Overload constructor that initializes the array size and allocates number of elements specified by size argument
		template <typename T>
		Array<T>::Array(const int& size_array)
		{
			...
			if (m_data == nullptr)   // Check what new returns to see if its NULL
				...
				std::cout << "Array Overload constructor." << endl;
		}
		...
		Array<T>::Array(const Array<T>& source)	// Copy constructor
		{
			...
			m_data = new T[source.m_size];  // Allocate a new C array with the same size
			...
			std::cout << "Array Copy constructor." << endl;
		}
		...
		Array<T>::~Array()  // Destructor
		{
			...
			//std::cout << "Array destructor." << endl;
		}
